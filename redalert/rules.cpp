...
RulesClass::RulesClass(void)
    ...
    , APMineDamage(1000)
    , MaxPlayers(8)
    , BaseDefenseDelay(1, 4)
    , SuspendPriority(20)
    , SuspendDelay(2)
    , SurvivorFraction(1, 2)
    , ReloadRate(".05")
    , AutocreateTime(5)
    , BuildupTime(".05")
    , OreDumpRate(2)
...
#endif
#ifdef FIXIT_ENGINEER //	checked - ajw 9/28/98
        //	Engineer changing fields were specifically left out of Aftrmath.ini, thus these values are not found to set.
        //ajw 	Implies interesting security hole if user creates a separate Aftrmath.ini file!
        EngineerDamage =
