                    ...
                    */
                    ...
                    if (*this == INFANTRY_SPY) {
                        ...
                        if (tech->What_Am_I() == RTTI_BUILDING) {
                            ...
                            if (build == STRUCT_RADAR || build == STRUCT_TDHQ || build == STRUCT_TDEYE) {
...
MoveType InfantryClass::Can_Enter_Cell(CELL cell, FacingType) const
{
    ...
    */
    ...
    while (obj != NULL) {
        ...
        if (obj != this) {
            ...
            */
            //				if (obj->What_Am_I() == RTTI_BUILDING || obj->What_Am_I() == RTTI_AIRCRAFT || obj->What_Am_I() ==
            //RTTI_UNIT) { 					if ((Mission == MISSION_CAPTURE || Mission == MISSION_SABOTAGE) && (obj->As_Target() ==
            //NavCom || obj->As_Target() == TarCom)) { 						return(MOVE_OK);
            //					}
            //				}
    ...
    */
    if (retval == MOVE_OK && !IsTethered && Ground[cellptr->Land_Type()].Cost[SPEED_FOOT] == 0) {
        ...
        ** should let him.
        */
        if (*this == INFANTRY_RENOVATOR && Is_Target_Cell(TarCom) && (cell == ::As_Cell(NavCom))
            && (cellptr->TType == TEMPLATE_BRIDGE1D || cellptr->TType == TEMPLATE_BRIDGE2D
                || (cellptr->TType >= TEMPLATE_BRIDGE_1C && cellptr->TType <= TEMPLATE_BRIDGE_3E))) {
            return (MOVE_OK);
        }
#endif
        return (MOVE_NO);
...
#endif
{
    ...
    if (Class->IsDog) {
        return (Coord_Spillage_List(
            Coord, 24 + (Doing == DO_DOG_MAUL ? 40 : 0) + (Doing >= DO_GUN_DEATH && Doing <= DO_FIRE_DEATH ? 40 : 0)));
    } else {
...
void InfantryClass::Scatter(COORDINATE threat, bool forced, bool nokidding)
{
    ...
    */
    if (!Rule.IsScatter && !nokidding && House->IsHuman && !forced && !Team.Is_Valid())
        return;
                ...
                if (object->What_Am_I() == RTTI_BUILDING) {
                    ...
                    while (*list != REFRESH_EOL && !found) {
                        ...
                        for (FacingType i = FACING_N; i < FACING_COUNT; i++) {
                            ...
                            if ((unsigned)adjcell >= MAP_CELL_TOTAL)
                                continue;
                    ...
                    if (infantry != NULL) {
                        ...
                        */
                        ...
                        if (infantry->Unlimbo(coord, dir)) {
                            ...
                            //						infantry->Strength = Fixed_To_Cardinal(infantry->Class_Of().MaxStrength,
                            //strength);
    ...
    */
    ...
    if (!IsFiring && !IsFalling && Doing != DO_DOG_MAUL) {
        if (!IsDriving) {
            ...
            */
            if ((!IsZoneCheat || Can_Enter_Cell(Coord_Cell(Coord)) != MOVE_NO) && !IsDriving && !IsTethered
                && Target_Legal(NavCom) && IsLocked
                        ...
                        if (Can_Enter_Cell(acell) == MOVE_DESTROYABLE) {
                            if (Map[acell].Cell_Object()) {
                                if (!House->Is_Ally(Map[acell].Cell_Object())) {
                                    Override_Mission(
                                        MISSION_ATTACK, Map[acell].Cell_Object()->As_Target(), TARGET_NONE);
                                }
                            } else {
