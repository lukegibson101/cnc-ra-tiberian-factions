...
TDAFLD = {
    ...
    "idle_anim":   (0, 16, 3),
    "active_anim": (0, 1, 0),
    "notes":       "TD Nod Airstrip. STOPGAP: Logic=WEAP reuses GDI Weapons Factory donor for vehicle-factory behaviour — units will emerge via WEAP's TD-SW exit track (gotchas #14-15) since RA has no native airstrip mechanic. Visually weird (no door on a flat tile, unit appears mid-airstrip) but functional. 4x2 footprint (AFLD preset in bdata.cpp added this session). idle_anim=(0,16,3) is TD-authentic (STRUCT_AIRSTRIP _anims[] in TD source) — TGA pack has 33 frames so plenty of headroom. active_anim clamped to (0,1,0) since WEAP donor's door-open ACTIVE animation doesn't apply to a doorless airstrip. Proper fix (cargo-plane choreography, OUT_OF_AIRSTRIP_TD enum + Track15 + new Exit_Object branch) deferred to dedicated session.",
}
